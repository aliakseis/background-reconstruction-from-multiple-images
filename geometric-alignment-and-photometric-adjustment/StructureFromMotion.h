...
private:
    ...
    void extractFeatures() {
        ...
        for (const auto& i : imagesFilenames) {
            ...
            //    imwrite(fs::basename(fs::path(i.c_str())) + "_features.jpg", out);
            //}
        }
    }
    ...
    std::vector<cv::DMatch> matchWithRatioTest(const cv::DescriptorMatcher& matcher, const cv::Mat& desc1, const cv::Mat& desc2) {
        using namespace cv;
        // Raw match
        std::vector< std::vector<DMatch> > nnMatch;
        matcher.knnMatch(desc1, desc2, nnMatch, 2);

        // Ratio test filter
        std::vector<DMatch> ratioMatched;
    ...
    void matchFeatures() {
        ...
        for (size_t i = 0; i < imagesFilenames.size() - 1; ++i) {
            for (size_t j = i + 1; j < imagesFilenames.size(); ++j) {
                const auto imgi = imagesFilenames[i];
                const auto imgj = imagesFilenames[j];

                // Match with ratio test filter
                auto match = matchWithRatioTest(matcher, descriptors[imgi], descriptors[imgj]);

                // Reciprocity test filter
                auto matchRcp = matchWithRatioTest(matcher, descriptors[imgj], descriptors[imgi]);
                std::vector<DMatch> merged;
                ...
                std::vector<uint8_t> inliersMask(merged.size());
                std::vector<Point2f> imgiPoints, imgjPoints;
    ...
    void buildTracks() {
        ...
        for (auto& v : tracks)
        {
            v.resize(goodComponents.size(), { -1, -1. });
        }
